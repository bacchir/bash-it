...
rafa_scm_prompt() {
        ...
        then
                ...
                echo "${bold_purple}($(scm_prompt_info))${normal}"
        fi
}
...
prompt() {
    ...
    # nice prompt
    case "`id -u`" in
        0) PS1="${TITLEBAR}┌─$(my_ve)$(chroot)$my_ps_root[${cyan}\w${normal}] $(rafa_scm_prompt)$(is_vim_shell)
└─▪ "
        ;;
        *) PS1="${TITLEBAR}┌─$(my_ve)$(chroot)$my_ps_user[${cyan}\w${normal}] $(rafa_scm_prompt)$(is_vim_shell)
└─▪ "
        ;;
    esac
}
